...
async function ensureAlarm() {
  ...
  const period = Math.max(1, cfg.checkPeriodMin);
  if (!existing || existing.periodInMinutes !== period) {
    // delayInMinutes 不限制最小值，让首次检查尽快发生；重复周期最小 1 分钟
    await chrome.alarms.create(ALARM_NAME, { delayInMinutes: 0.2, periodInMinutes: period });
  }
}
...
async function fetchSession() {
  ...
  if (!data || !data.accessToken) throw new Error('未登录或会话已失效');
  return data;
}
...
async function tick() {
  try {
    ...
    if (Date.now() - lastPushAt >= cfg.pushIntervalHours * 3600 * 1000) {
      await pushNow('auto').catch(() => { /* 失败已记录到 state */ });
    }
    setBadge('');
  } catch (e) {
    await patchState({
  ...
  (async () => {
    switch (msg && msg.type) {
      case 'get_snapshot':
        ...
        break;
      case 'save_config':
        await chrome.storage.local.set({ config: { ...DEFAULTS, ...(msg.config || {}) } });
      ...
      case 'manual_push':
        try {
          const text = await pushNow('manual');
